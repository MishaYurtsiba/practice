...
const faqModel = {
  ...
  update: function (faq, id) {
    //faq = { question: "Найкраща спеціальність?",} id = 5
    ...
    // ...
    for (let key of this.keys) if (faq[key]) element[key] = faq[key];
    return element;
  },
  delete: function (id) {
    const index = this.data.findIndex((f) => f.id == id);
    if (index == -1) return null;
    const deletedElement = this.data.splice(index, 1)[0];
